...
def evaluate(estimator, x_test, y_test):
    """Estimates y using a estimator and returns the average SSE relative to the y_test"""

    # y_pred = estimator.predict(x_test)
    y_pred = toxic_algae_model(X_test=x_test, intercept=estimator.intercept_, coefs=estimator.coef_)

    # return compute_SEE(y_predicted=y_pred, y_real=y_test)
...
def compare_models(
    ...
    ridge_reg_avg_scores_sse = []
    ridge_reg_avg_scores_r2 = []
    for alpha in lambdas:
        ...
        ridge_reg_scores_sse = cross_validate(
            estimator=ridge_reg,
            X=X_clean,
            y=y_clean,
            cv=NUM_FOLDS,
        ...
        ridge_reg_avg_scores_sse.append(sum(ridge_reg_scores_sse) / NUM_FOLDS)

        # Perform cross-validation and calculate average score
        ridge_reg_scores_r2 = cross_validate(
            estimator=ridge_reg,
            X=X_clean,
            y=y_clean,
            cv=NUM_FOLDS
        )["test_score"]
        ridge_reg_avg_scores_r2.append(sum(ridge_reg_scores_r2) / NUM_FOLDS)
    
    # Save best R²
    max_ridge_avg_scores_r2 = max(ridge_reg_avg_scores_r2)
    
    # Find best lambda value and corresponding score for Ridge Regression
    min_ridge_avg_scores_sse = min(ridge_reg_avg_scores_sse)
    best_ridge_lambda = lambdas[ridge_reg_avg_scores_sse.index(min_ridge_avg_scores_sse)]

    # Refit with best found lambda to get access to the best coefs and intercept
    ridge_reg = Ridge(alpha=best_ridge_lambda, fit_intercept=True, max_iter=5000).fit(X=X_clean, y=y_clean)
    ...
    print(f"\tBest average score (R²) = {max_ridge_avg_scores_r2} (lambda = {best_ridge_lambda})")
    print(f"\tBest average score (SSE) = {min_ridge_avg_scores_sse} (lambda = {best_ridge_lambda})")
    print(f"\tPrediction score for inliers (SSE) = {ridge_reg_sse} (lambda = {best_ridge_lambda})")
    ...
    for alpha in lambdas:
        ...
        lasso_reg_avg_scores_sse.append(sum(lasso_reg_scores_sse) / NUM_FOLDS)

        # Perform cross-validation and calculate average score
        lasso_reg_scores_r2 = cross_validate(
            estimator=lasso_reg,
            X=X_clean,
            y=y_clean,
            cv=NUM_FOLDS
        )["test_score"]
        lasso_reg_avg_scores_r2.append(sum(lasso_reg_scores_r2) / NUM_FOLDS)
    
    # Save best R²
    max_lasso_avg_scores_r2 = max(lasso_reg_avg_scores_r2)
    
    # Find best lambda value and corresponding score for Lasso Regression
    min_lasso_avg_scores_sse = min(lasso_reg_avg_scores_sse)
    best_lasso_lambda = lambdas[lasso_reg_avg_scores_sse.index(min_lasso_avg_scores_sse)]

    # Refit with best found lambda to get access to the best coefs and intercept
    lasso_reg = Lasso(alpha=best_lasso_lambda, fit_intercept=True, max_iter=5000).fit(X=X_clean, y=y_clean)
    ...
    print(f"\tBest average score (R²) = {max_lasso_avg_scores_r2} (lambda = {best_lasso_lambda})")
    print(f"\tBest average score (SSE) = {min_lasso_avg_scores_sse} (lambda = {best_lasso_lambda})")
    print(f"\tPrediction score for inliers (SSE) = {lasso_reg_sse} (lambda = {best_lasso_lambda})")
    print(f"\tModel parameters:")
    ...
    elastic_net_reg_avg_scores_sse = []
    elastic_net_reg_avg_scores_r2 = []
    min_elastic_net_avg_scores_sse = 0
    for alpha in lambdas:
        for ratio in l1_ratio:
            ...
            # Perform cross-validation and calculate average score
            elastic_net_reg_scores_sse = cross_validate(
            ...
            elastic_net_reg_avg_scores_sse.append(sum(elastic_net_reg_scores_sse) / NUM_FOLDS)

            # Perform cross-validation and calculate average score
            elastic_net_reg_scores_r2 = cross_validate(
                estimator=elastic_net_reg,
                X=X_clean,
                y=y_clean,
                cv=NUM_FOLDS,
            ...
            elastic_net_reg_avg_scores_r2.append(sum(elastic_net_reg_scores_r2) / NUM_FOLDS)
            
            # Track the best ElasticNet model based on average score
            if min(elastic_net_reg_avg_scores_sse) > min_elastic_net_avg_scores_sse:
                best_elastic_net_lambda = alpha
                best_elastic_net_ratio = ratio


    # Refit with best found lambda to get access to the best coefs and intercept
    elastic_net_reg = ElasticNet(
        alpha=best_elastic_net_lambda, 
        fit_intercept=True, 
        l1_ratio=best_elastic_net_ratio, 
        max_iter=6000
    ...
    print(f"\tBest average score (R²) = {max(elastic_net_reg_avg_scores_r2)} (lambda = {best_elastic_net_lambda}, l1_ratio = {best_elastic_net_ratio})")
    print(f"\tBest average score (SSE) = {min(elastic_net_reg_avg_scores_sse)} (lambda = {best_elastic_net_lambda}, l1_ratio = {best_elastic_net_ratio})")
    print(f"\tPrediction score for inliers (SSE) = {elastic_net_reg_sse} (lambda = {best_elastic_net_lambda}, l1_ratio = {best_elastic_net_ratio})")
    print(f"\tModel parameters:")
    ...
    r_squared = [max_ridge_avg_scores_r2, max_lasso_avg_scores_r2, max(elastic_net_reg_avg_scores_r2)]

    # Store the SSE scores for comparison inliers
    sse_inliers = [lasso_reg_sse, ridge_reg_sse, elastic_net_reg_sse]

    # Store the SSE scores for comparison
    sse = [min_lasso_avg_scores_sse, min_ridge_avg_scores_sse, min(elastic_net_reg_avg_scores_sse)]
    
    # Determine the best model based on R²
    if max(r_squared) == max_ridge_avg_scores_r2:
        print("---------------------------")
        print(f"Best model is Ridge Regression with R² = {max_ridge_avg_scores_r2}")
    elif max(r_squared) == max_lasso_avg_scores_r2:
        print("---------------------------")
        print(f"Best model is Lasso Regression with R² = {max_lasso_avg_scores_r2}")
    elif max(r_squared) == max(elastic_net_reg_avg_scores_r2):
        print("---------------------------")
        print(f"Best model is ElasticNet Regression with R² = {max(elastic_net_reg_avg_scores_r2)}")

   # Determine the best model based on SSE
    if min(sse) == min_lasso_avg_scores_sse:
        print("---------------------------")
        print(f"Best model is Ridge Regression with SSE = {min_lasso_avg_scores_sse}")
    elif min(sse) == min_ridge_avg_scores_sse:
        print("---------------------------")
        print(f"Best model is Lasso Regression with SSE = {min_ridge_avg_scores_sse}")
    elif min(sse) == min(elastic_net_reg_avg_scores_sse):
        print("---------------------------")
        print(f"Best model is ElasticNet Regression with SSE = {min(elastic_net_reg_avg_scores_sse)}")

    # Determine the best model based on SSE (inliers)
    if min(sse_inliers) == ridge_reg_sse:
        print("---------------------------")
        print(f"Best model is Ridge Regression with SSE (inliers) = {ridge_reg_sse}")
    elif min(sse) == lasso_reg_sse:
        print("---------------------------")
        print(f"Best model is Lasso Regression with SSE (inliers) = {lasso_reg_sse}")
    elif min(sse) == elastic_net_reg_sse:
        print("---------------------------")
        print(f"Best model is ElasticNet Regression with SSE (inliers) = {elastic_net_reg_sse}")
    ...
    ax1.plot(lambdas, ridge_reg_avg_scores_r2)
    ...
    ax2.plot(lambdas, ridge_reg_avg_scores_sse)
    ...
    ax2.set_ylabel("Average SSE")

    # Save the figure for Ridge Regression
    plt.tight_layout()
    plt.savefig(get_plot_save_path("Ridge Regression SSE"), bbox_inches='tight')

    ###############################
    # Plot Lasso Regression results
    ###############################
    fig3, ax3 = plt.figure(), plt.gca()
    ax3.plot(lambdas, lasso_reg_avg_scores_r2)

    y_min_3, y_max_3 = ax3.get_ylim()
    ax3.set_yticks(np.linspace(y_min_3, y_max_3, 5))
    ...
    ax4.plot(lambdas, lasso_reg_avg_scores_sse)

    y_min_4, y_max_4 = ax4.get_ylim()
    ax4.set_yticks(np.linspace(y_min_4, y_max_4, 5))
    ...
    # Save the figure for Lasso Regression
    plt.tight_layout()
    plt.savefig(get_plot_save_path("Lasso Regression SSE"), bbox_inches='tight')
...
def main() -> None:
    ...
    X_test = load_data(
        filename=get_absolute_path(file_name="X_test.npy")
    )  # Test data for the model
    y_train = load_data(
        filename=get_absolute_path(file_name="y_train.npy")
    )  # Expected output for the training data
    X_train = load_data(
        filename=get_absolute_path(file_name="X_train.npy")
