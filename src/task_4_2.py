...
def teste_alphas(X_train: np.ndarray, Y_train: np.ndarray):
    ...
    ridge_reg = Ridge(alpha=max_ridge_lambda, fit_intercept=True, max_iter=5000).fit(X=X_train, y=Y_train)
...
def main():
    ...
    # Save last 400 samples resulting from the predicted y_test
    y_pred_last_400 = Y_pred[-400:]  
    save_npy_to_output("4.1", y_pred_last_400)
